...
class TestMaestroTools:
    ...
    def test_add_items_to_batch(self):
        """Should add items to an existing batch."""
        # Create batch first
        batch = MaestroTools.create_batch(CreateBatchInput(name="Test"))
    ...
    def test_get_batch_returns_dto(self):
        """Should retrieve batch details by ID."""
        # Create batch
        batch = MaestroTools.create_batch(CreateBatchInput(name="Retrieve Test"))
    ...
    def test_get_batch_invalid_id_raises_error(self):
        """Should raise error for nonexistent batch."""
        input_data = GetBatchInput(batch_id="invalid-id")
        
        with pytest.raises(KeyError):
            MaestroTools.get_batch(input_data)
    
    def test_list_batches_without_filter(self):
        """Should list all batches."""
        # Create multiple batches
        MaestroTools.create_batch(CreateBatchInput(name="Batch 1"))
        MaestroTools.create_batch(CreateBatchInput(name="Batch 2"))
    ...
    def test_list_batches_filters_by_status(self):
        """Should filter batches by status."""
        # Create and start a batch
        batch = MaestroTools.create_batch(CreateBatchInput(name="Started Batch"))
        MaestroTools.start_batch(StartBatchInput(batch_id=batch.batch_id))
    ...
    def test_start_batch_updates_status(self):
        """Should start a batch and update status."""
        batch = MaestroTools.create_batch(CreateBatchInput(name="Start Test"))
    ...
    def test_complete_batch_updates_status(self):
        """Should complete a batch."""
        batch = MaestroTools.create_batch(CreateBatchInput(name="Complete Test"))
        MaestroTools.start_batch(StartBatchInput(batch_id=batch.batch_id))
    ...
    def test_cancel_batch_succeeds(self):
        """Should cancel a batch."""
        batch = MaestroTools.create_batch(CreateBatchInput(name="Cancel Test"))
